...
public class MovieAdd extends ListActivity {
		...
		try {
			...
			if(!movies.isEmpty())
			{
				...
				for(Movie movie : movies)
				{
					...
					map.put("title", movie.getName() + " (" + Integer.toString(movie.getReleasedDate().getYear()+1900) + ")");
					list.add(map);
				}
				...
				SimpleAdapter adapter = new SimpleAdapter(this.getApplicationContext(), list, R.layout.movie_add_row, from, to);
	...
	protected void saveMovieToDb(int iD) {
		try {
			...
			mDbHelper.createMovie(movie.getName(),Integer.toString(movie.getReleasedDate().getYear()+1900),genreString,movie.getOverview(),posterurl,movie.getTrailer().toString());
			setResult(RESULT_OK);
            finish();
